...
for (const lang of languages) {
  ...
  if (parts.length === 2) {
    ...
    // We replace the FIRST occurrence of "quranUI": { in this block
    if (block.match(/"quranUI"\s*:\s*\{/)) {
      // Check if we haven't already injected showTafsir
      ...
      // Create quranUI if it doesn't exist for some reason
      block = `\n  "quranUI": {\n    "showTafsir": "${trans.showTafsir}",\n    "accessToTafseer": "${trans.accessToTafseer}"\n  },` + block;
    }
    ...
    if (block.match(/"sunnahUI"\s*:\s*\{/)) {
      ...
      // Create sunnahUI if it doesn't exist
      block = `\n  "sunnahUI": {\n    "showTranslations": "${trans.showTranslations}"\n  },` + block;
    }
...
console.log('Successfully injected showTafsir, accessToTafseer, and showTranslations into existing UI objects.');
